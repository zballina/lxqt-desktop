...
#include <lxqt/lxqtaboutdialog.h>
      ...
      m_activePlugin(0)
{
    ...
    foreach (QString configId, plugins)
    {
        ...
        LxQt::PluginInfoList list = LxQt::PluginInfo::search(desktopDirs, "RazorDesktop/Plugin", QString("%1.desktop").arg(libName));
        if( !list.count())
        {
...
void DesktopScene::wheelEvent(QGraphicsSceneWheelEvent * e)
{
    ...
    if (!plug) {
        ...
        int max = LxQt::xfitMan().getNumDesktop() - 1;
        int delta = e->delta() > 0 ? 1 : -1;
        int current = LxQt::xfitMan().getActiveDesktop() + delta;
        ...
        LxQt::xfitMan().setActiveDesktop(current);
    }
...
{
    LxQt::AboutDialog dia;
}

void DesktopScene::showAddPluginDialog()
{
    LxQt::AddPluginDialog* dlg = findChild<LxQt::AddPluginDialog*>();

    if (!dlg)
    {
        dlg = new LxQt::AddPluginDialog(pluginDesktopDirs(), "RazorDesktop/Plugin", "*");
//        dlg->move(sceneRect().center() - dlg->sceneRect().center());
        dlg->setAttribute(Qt::WA_DeleteOnClose);
        connect(dlg, SIGNAL(pluginSelected(const LxQt::PluginInfo&)),
                this, SLOT(addPlugin(const LxQt::PluginInfo&)));
...
void DesktopScene::addPlugin(const LxQt::PluginInfo &pluginInfo)
{
...
DesktopWidgetPlugin * DesktopScene::loadPlugin(QLibrary * lib, const QString & configId, const LxQt::PluginInfo &info)
{
    Q_ASSERT(lib);
...
QLibrary* DesktopScene::loadPluginLib(const LxQt::PluginInfo &pluginInfo)
{
    QLibrary* lib = 0;
