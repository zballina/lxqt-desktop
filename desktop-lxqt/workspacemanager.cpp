...
RazorWorkSpaceManager::RazorWorkSpaceManager(const QString & configId, LxQt::Settings * config)
    : DesktopPlugin(configId, config)
    , m_scene(0)
    , m_desktopCount(1)
{
    LxQt::Translator::translateLibrary("desktop-razor");
    ...
    setup();
    connect(lxqtApp, SIGNAL(themeChanged()), this, SLOT(setup()));
}
...
void RazorWorkSpaceManager::setup()
{
    ...
    for (int screen = 0; screen < numScreens; ++screen)
    {
       ...
       for (int desktop = 0; desktop < m_desktopCount; ++desktop)
       {
           ...
           themeWallpaper = LxQt::razorTheme.desktopBackground(desktop + 1);
...
//            qDebug() << "virtual desktop: " << desktop;
            ...
            if (useDifferentWallpapers)
                LxQt::xfitMan().moveWindowToDesktop(ws->winId(), desktop);
            else
                LxQt::xfitMan().moveWindowToDesktop(ws->winId(), 0xFFFFFFFF);
        }
    }
}
