...
RazorDeskManager::RazorDeskManager(const QString & configId, LxQt::Settings * config)
    : DesktopPlugin(configId, config)
{
    ...
    if (! finalPixmap.isEmpty())
    {
        ...
        XChangeProperty(QX11Info::display(), QX11Info::appRootWindow(), LxQt::XfitMan::atom("_XROOTPMAP_ID"), XA_PIXMAP, 32, PropModeReplace, (unsigned char *) &p, 1);
        XChangeProperty(QX11Info::display(), QX11Info::appRootWindow(), LxQt::XfitMan::atom("ESETROOT_PMAP_ID"), XA_PIXMAP, 32, PropModeReplace, (unsigned char *) &p, 1);
    ...
    if (makeIcons)
    {
        deskicons = new LxQt::Settings("deskicons", this);
