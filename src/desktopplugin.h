...
public:
    ...
    DesktopPlugin(const QString & configId, LxQt::Settings * config)
        : m_config(config)
    {
    };
 ...
 */
typedef DesktopPlugin* (*PluginInitFunction)(const QString & configId, LxQt::Settings * config);
...
#define EXPORT_RAZOR_DESKTOP_PLUGIN_H \
    extern "C" DesktopPlugin* init(const QString & configId, LxQt::Settings * config);

/*! Helper macro for define RazorPanelPlugin.
    Place this macro in your plugin source file.
 */
#define EXPORT_RAZOR_DESKTOP_PLUGIN_CPP(PLUGINCLASS)        \
    DesktopPlugin* init(const QString & configId, LxQt::Settings * config) \
    {                                                       \
